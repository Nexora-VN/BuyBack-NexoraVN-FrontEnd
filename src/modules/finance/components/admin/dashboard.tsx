...
export function AdminDashboardPage() {
  ...
  return (
    <Page title={t("Tổng quan vận hành")}>
      {query.isLoading ? (
        <Loading />
      ) : query.isError ? (
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {[
            [
              "pendingWithdrawals",
              t("Yêu cầu rút chờ xử lý"),
              "/admin/withdrawals?withdrawals-status=PENDING",
            ],
            [
              "pendingBanks",
              t("Ngân hàng chờ duyệt"),
              "/admin/bank-accounts?bank-accounts-status=PENDING",
            ],
            ...
            <Link key={key} href={href}>
              <StatCard
                label={t(label)}
                value={
                  query.data?.operations?.[key as keyof NonNullable<Dashboard["operations"]>] ?? "—"
                }
              />
            </Link>
          ))}
