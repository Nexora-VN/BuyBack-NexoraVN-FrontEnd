...
export const credentialFields = [
  ...
  {
    name: "expectedAffiliate",
    label: "Tên Affiliate kỳ vọng",
    help: "Tên affiliate xuất hiện trong báo cáo API AddLiveTag",
  },
];
...
export const verificationFields = [
  {
    name: "version",
    label: "Phiên bản Credential (Version)",
    type: "number" as const,
    help: "Nhập số phiên bản credential hiện tại",
  },
  {
    name: "evidence",
    label: "Bằng chứng đối chiếu thực tế",
    type: "textarea" as const,
    help: "Ghi rõ nguồn chứng từ đối chiếu thực tế (tối thiểu 10 ký tự) để mở khóa settlement",
  },
];
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
      <StatCard
        ...
        value={
          <StatusBadge
            status={typeof shopeeCred?.status === "string" ? shopeeCred.status : "UNVERIFIED"}
          />
        }
      ...
      <StatCard
        ...
        value={text(query.data, "provider") || "ADDLIVETAG"}
        helper={`Account: ${text(cred, "accountId")} · Affiliate: ${text(cred, "expectedAffiliate")}`}
      />
      <StatCard
        label="Xác thực VND & Account"
        value={
          isVerified ? (
            <span className="text-success font-semibold">{t("Đã xác thực")}</span>
          ) : (
            <span className="text-warning font-semibold">{t("Chưa xác thực")}</span>
          )
        }
        helper={
          isVerified
            ? t("Đã mở khóa settlement")
            : t("Khóa settlement tới khi SUPER_ADMIN xác nhận")
        }
      />
      <StatCard
        label="Batch & Vấn đề"
        value={t("{value0} vấn đề mở", { value0: text(query.data, "openIssues") })}
        helper={t("Chờ/chạy: {value0} · Lỗi: {value1}", {
          value0: text(query.data, "queuedOrRunning"),
          value1: text(query.data, "failedBatches") || "0",
        })}
      />
      <StatCard
        label="Ledger & Đồng bộ"
        value={text(query.data, "ledgerBalanced") === "true" ? t("Cân bằng") : t("Cần kiểm tra")}
        helper={
          text(query.data, "syncLagSeconds") === "—"
            ? t("Chưa đồng bộ")
            : t("Sync cách {value0}s", { value0: text(query.data, "syncLagSeconds") })
        }
      />
    </div>
  );
...
export function ProviderSyncPage() {
  ...
  return (
    <Page title={t("Kết nối Đối tác & Shopee")}>
      <ProviderHealth />
      {superAdmin && (
        <>
          <MutationForm
          ...
          <MutationForm
            title={t("Cập nhật kết nối AddLiveTag")}
            path="admin/provider-credential"
            method="put"
          ...
          >
            <p className="text-muted-foreground text-sm">
              {t(
                "API key chỉ cấu hình qua biến môi trường ADDLIVETAG_API_KEY của backend. Đổi key cần khởi động lại backend; không nhập hoặc lưu key trên web/database. Form này chỉ cập nhật thông tin tài khoản.",
              )}
            </p>
          </MutationForm>
          <MutationForm
            title={t("Xác thực tài khoản & đơn vị tiền VND")}
            path="admin/provider-credential/verify"
            method="post"
            fields={verificationFields}
            schema={providerVerificationSchema}
          >
            <p className="text-muted-foreground text-sm">
              {t(
                "SUPER_ADMIN xác nhận account ID và đơn vị tiền integer VND bằng đối chiếu thực tế kèm bằng chứng để mở khóa settlement.",
              )}
            </p>
          </MutationForm>
        </>
      ...
      >
        <p className="text-muted-foreground text-sm">
          {t(
            "Khoảng ngày dài hơn 90 ngày sẽ được hệ thống tự động chia thành các batch độc lập không chồng lấn.",
          )}
        </p>
      </MutationForm>
    </Page>
