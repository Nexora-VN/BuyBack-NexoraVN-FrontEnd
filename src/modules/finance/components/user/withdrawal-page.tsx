...
export function NewWithdrawalPage(){
 ...
 function validate(){const result=withdrawalSchema.safeParse({amount,bankId});if(!result.success){setErrors(Object.fromEntries(result.error.issues.map(i=>[String(i.path[0]),i.message])));return;}if(!wallet.data||BigInt(amount)>BigInt(wallet.data.available)){setErrors({amount:'Số dư khả dụng không đủ'});return;}setErrors({});setStep('review');}
 async function send(){if(lock.current)return;lock.current=true;setPending(true);setError('');key.current??=crypto.randomUUID();try{await financeService.mutate('me/withdrawals',{amount,bankId,idempotencyKey:key.current});await refresh();key.current=null;setStep('done');}catch(e){setError(e instanceof Error?e.message:'Không thể tạo yêu cầu');}finally{lock.current=false;setPending(false);}}
 return <Page title="Yêu cầu rút tiền" description="Tối thiểu 50.000 VND. Số tiền được giữ ngay khi gửi yêu cầu.">
 {banks.isLoading||wallet.isLoading?<Loading/>:banks.isError||wallet.isError?<Failure message={(banks.error||wallet.error)?.message||'Không thể tải dữ liệu'} retry={()=>{void banks.refetch();void wallet.refetch();}}>:<Card className="w-full max-w-2xl">
