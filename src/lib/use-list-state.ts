...
export function useListState(scope:string) {
 ...
 function update(values:Partial<{page:number;status:string;query:string;sort:'asc'|'desc'|string}>) {
  const url=new URL(window.location.href);
  for(const [key,value] of Object.entries(values)){const name=prefix+(key==='query'?'q':key);if((value===''&&key!=='status')||value===1||key==='sort'&&value==='desc')url.searchParams.delete(name);else url.searchParams.set(name,String(value));}
