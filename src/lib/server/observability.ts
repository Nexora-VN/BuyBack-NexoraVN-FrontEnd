...
export function serverLog(event: string, fields: Record<string, string | number | undefined>) {
  ...
  console.log(
    JSON.stringify({
      level:
        typeof fields.statusCode === "number" && fields.statusCode >= 500
          ? 50
          : typeof fields.statusCode === "number" && fields.statusCode >= 400
            ? 40
            : 30,
      time: Date.now(),
      event,
...
export function withApiRoute<T>(handler: (request: Request, context: T) => Promise<Response>) {
  return (request: Request, context: T) =>
    requestContext.run(
      { requestId: requestId(request.headers.get("x-request-id")), signal: request.signal },
      async () => {
        ...
        if (response.status >= 400) {
          ...
          try {
            ...
            /* Non-JSON failures must not expose HTML or provider errors. */
          }
          response = NextResponse.json(
            {
              statusCode: response.status,
              code: typeof body.code === "string" ? body.code : "REQUEST_FAILED",
              message: typeof body.message === "string" ? body.message : "Không thể xử lý yêu cầu.",
              requestId: id,
              timestamp: new Date().toISOString(),
              path,
              ...(Array.isArray(body.details) ? { details: body.details } : {}),
            },
        ...
        if (!path.includes("/health") || response.status >= 400)
          serverLog("http.completed", {
            path,
...
export async function backendFetch(url: string | URL, init: RequestInit = {}): Promise<Response> {
  ...
  try {
    ...
    try {
      ...
      throw new UpstreamError("UPSTREAM_INVALID_RESPONSE", 502);
    }
    return new Response(text, { status: response.status, headers: response.headers });
  } catch (error) {
    if (error instanceof UpstreamError) throw error;
    if (timeout.aborted) throw new UpstreamError("UPSTREAM_TIMEOUT", 504);
    if (context?.signal.aborted || init.signal?.aborted)
      throw new UpstreamError("REQUEST_CANCELLED", 499);
