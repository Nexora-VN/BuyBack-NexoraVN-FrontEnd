...
const schema = z.object({ name: z.enum(['Error', 'TypeError', 'RangeError', 'ReferenceError', 'SyntaxError', 'ApiError']), source: z.enum(['boundary', 'error', 'unhandledrejection']), path: z.string().max(500).startsWith('/'), requestId: z.string().refine(validRequestId).optional() }).strict();
export const POST = withApiRoute(async (request: Request) => {
  if (request.headers.get('origin') !== new URL(request.url).origin) return NextResponse.json({ message: 'Same-origin required' }, { status: 403 });
  const now = Date.now();
  // Only trust a proxy-provided address when the operator explicitly configures the boundary.
  const key = process.env.TRUST_PROXY_IP === 'true' ? (request.headers.get('x-forwarded-for')?.split(',')[0]?.trim().slice(0, 64) ?? 'unknown') : 'unknown';
  ...
  let size = 0;
  try {
    for (;;) { const chunk = await reader.read(); if (chunk.done) break; size += chunk.value.byteLength; if (size > 8192) return new NextResponse(null, { status: 413 }); chunks.push(chunk.value); }
  } finally { await reader.cancel(); }
  let raw: unknown;
  try { raw = JSON.parse(Buffer.concat(chunks).toString('utf8')); } catch { return new NextResponse(null, { status: 400 }); }
